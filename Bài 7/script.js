...
function addToCart(productId) {
    ...
    if (product) {
        ...
        let newProduct = products.find(p => p.id === productId);
...
function updateCart() {
    ...
    if (cart.length === 0) {
        ...
        cart.forEach(item => {
            ...
            textDiv.innerHTML = `
                ...
                <span style="color: #E74753;">${item.price.toLocaleString('vi-VN').replace(/,/g, '.')}₫</span> 
                <span style="color: #E74753;"> x ${item.quantity}</span>  
            `;
            ...
            buttonDiv.innerHTML = `
                <button onclick="decreaseQuantity(${item.id})" style="background-color: #3D8C40; color: white; padding: 5px 10px; border: none; border-radius: 5px;">-</button>
                <span style="color: white;">${item.quantity}</span>  
                <button onclick="increaseQuantity(${item.id})" style="background-color: #3D8C40; color: white; padding: 5px 10px; border: none; border-radius: 5px;">+</button>
                <button onclick="removeFromCart(${item.id})" style="background-color: #9A0E19; color: white; padding: 5px 10px; border: none; border-radius: 5px;">X</button>
            `;
    ...
    totalElement.textContent = "Tổng: " + total.toLocaleString('vi-VN').replace(/,/g, '.') + "₫";
}
...
function checkout() {
    ...
    alert(`Cảm ơn bạn đã mua hàng!\nTổng giá trị đơn hàng: ${total.toLocaleString('vi-VN').replace(/,/g, '.')}₫`);
    cart = [];
    updateCart();
}
displayProducts();    